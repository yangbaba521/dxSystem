 ...
 */
@WebServlet(name = "getTeamInfo")
public class getTeamInfo extends HttpServlet {
    Logger logger = Logger.getLogger(getTeamInfo.class);
        ...
        try{
            ...
            try{
                ...
                if(resNum <= 0 || (!(result.get(0).equals("root")) && !(result.get(0).equals("charge")) )){
                    ...
                    result.clear();
                    mysqlStr="select * from team where teamCode='"+ teamCode+"';";
                    resNum = mysql.doQuery(mysqlStr,result);
                    if(resNum > 0){
                        result.remove(0);
                        data.put("result","success");
                        data.put("teamInfo",result);
                    }else{
                        data.put("result","faill");
                        data.put("cause", "无法找到该部组");
                    }
                }
            }catch(Exception e){
                e.printStackTrace();
                ...
                data.put("result","faill");
                data.put("cause", "无法找到该部组,请再次尝试,或者联系管理员");
