 ...
 */
@WebServlet(name = "getPartInfo")
public class getPartInfo extends HttpServlet {
    Logger logger = Logger.getLogger(getPartInfo.class);
        ...
        try{
            ...
            try{
                ...
                if(resNum <= 0 || (!(result.get(0).equals("root")) && !(result.get(0).equals("charge")) )){
                    ...
                    result.clear();
                    mysqlStr="select * from part where partCode='"+ partCode+"';";
                    resNum = mysql.doQuery(mysqlStr,result);
                    if(resNum > 0){
                        result.remove(0);
                        data.put("result","success");
                        data.put("partInfo",result);
                    }else{
                        data.put("result","faill");
                ...
                data.put("result","faill");
                data.put("cause", "无法找到该区部,请再次尝试,或者联系管理员");
