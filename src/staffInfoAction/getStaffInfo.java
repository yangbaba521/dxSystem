...
import teamInfoAction.getTeamInfo;
 ...
 */
@WebServlet(name = "getStaffInfo")
public class getStaffInfo extends HttpServlet {
    Logger logger = Logger.getLogger(getStaffInfo.class);
        ...
        try{
            ...
            try{
                ...
                if(resNum <= 0 || (!(result.get(0).equals("root")) && !(result.get(0).equals("charge")) )){
                    ...
                    result.clear();
                    mysqlStr="select * from staff where staffCode='"+ staffCode+"';";
                    resNum = mysql.doQuery(mysqlStr,result);
                    if(resNum > 0){
                        result.set(8,enc.convertMD5(result.get(8))+"");
                        result.set(18,enc.convertMD5(result.get(18))+"");
                        result.set(21,enc.convertMD5(result.get(21))+"");
                        result.remove(0);
                        data.put("result","success");
                        data.put("staffInfo",result);
                    }else{
                        data.put("result","faill");
                        data.put("cause", "无法找到该组员");
                    }
                }
            }catch(Exception e){
                e.printStackTrace();
                ...
                data.put("result","faill");
                data.put("cause", "无法找到该组员,请再次尝试,或者联系管理员");
